...
parfor i=1:size(image_paths,1),
    ...
    %experiment with smoothing before this step
    %image= vl_imsmooth(image,5);
    [locations, SIFT_features] = vl_dsift(image,'step',30);
    
    %if i==1,
        sift=horzcat(sift,SIFT_features);
...
[centers, assignments] = vl_kmeans(single(sift), vocab_size);
size(centers')
vocab=centers';
