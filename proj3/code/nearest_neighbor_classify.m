...
[indices,distances]=knnsearch(train_image_feats,test_image_feats,'K',7);
distances=1./distances;

indices_final=zeros(size(indices,1),1);

parfor i=1:size(indices,1),
    
   unique_vals=unique(indices(i,:));
   max_ind=0;
   max_val=0;
   
   for j=unique_vals,
       weight=sum(distances(i,find(indices(i,:)==j)));
   ...
   indices_final(i)=max_ind;

end
predicted_categories={train_labels{indices_final}}';
