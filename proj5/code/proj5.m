...
if ~exist('forestNoSS.mat','file')
    [img_features, labels] = get_sketch_tokens( train_img_dir, train_gt_dir, feature_params, num_sketch_tokens,self_sim_grid_dim);
end
'features retrieved'
...
if ~exist('forestNoSS.mat', 'file')
    ...
    error('Number of sketch tokens does not equal number of non-background categories')
        ...
        fprintf('No existing forest found. Computing one from training images\n')
        fprintf('Training random forest\n')
        pTrain={'M',20};
        tic, forest=forestTrain( img_features, labels, pTrain); toc
        save('forestNoSS.mat', 'forest')
end
load('forestNoSS.mat')
...
for i = 1:num_sketch_tokens
    ...
    plot([0 size(bg_confs,1)], [0 0], 'b');http://cs.brown.edu/courses/csci1430/proj5/
    hold off;
...
for f=1:length(test_imgs)
    ...
    [pb] = detect_sketch_tokens(cur_img, forest, feature_params,self_sim_grid_dim);
    
    %call stToEdges here or inside 'detect_sketch_tokens'
    pb=stToEdges(pb,true,true);
     %figure(201)
     %imagesc(pb)
     %figure(1);imshow(pb);pause(0.01);
