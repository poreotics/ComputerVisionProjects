...
data_path = '/course/cs143/asgn/proj4/data'; %change if you want to work with a network copy
...
if ~exist('pos.mat', 'file')
        fprintf('No existing positive features found. Computing one from training images\n')
        pos = get_positive_features( train_path_pos, feature_params );
        save('pos.mat', 'pos')
end
if ~exist('neg2.mat', 'file')
        fprintf('No existing negative features found. Computing one from training images\n')
        num_negative_examples = 10000;
        neg = get_random_negative_features( non_face_scn_path, feature_params, num_negative_examples);
        save('neg2.mat', 'neg')
end
load('pos.mat')
load('neg2.mat')
features_pos = pos;%get_positive_features( train_path_pos, feature_params );
features_neg = neg;%get_random_negative_features( non_face_scn_path, feature_params, num_negative_examples);
...
labels=ones(1,size(features,2));
labels(size(features_pos,1)+1:end)=-1;
[w b] = vl_svmtrain(features,labels, .1);
